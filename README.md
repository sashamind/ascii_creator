...
- **Contour mode** — a Sobel edge-detection pass that draws just the outlines with line characters (`| - / \`) oriented along each edge, with an adjustable threshold
...
- Контурный режим: детекция краёв по Собелю — рисуются только контуры символами линий (`| - / \`) по направлению края, с настраиваемым порогом
